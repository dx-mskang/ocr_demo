...
if [[ -n "$DX_RT_PATH" ]]; then
    ...
    log "⚠ Error DX_RT path not specified"
    exit 1
fi
...
# =============================================================================
# DXNN Model Assets Check
# =============================================================================

log "=== Checking DXNN model assets ==="
MODELS_DIR="$PROJECT_ROOT/engine/models"
DXNN_DIR="$MODELS_DIR/dxnn_optimized"
DXNN_MOBILE_DIR="$MODELS_DIR/dxnn_mobile_optimized"

NEED_SETUP=0
if [[ ! -d "$DXNN_DIR" ]]; then
    log "⚠ Missing model folder: $DXNN_DIR"
    NEED_SETUP=1
fi
if [[ ! -d "$DXNN_MOBILE_DIR" ]]; then
    log "⚠ Missing model folder: $DXNN_MOBILE_DIR"
    NEED_SETUP=1
fi

if [[ $NEED_SETUP -eq 1 ]]; then
    if [[ -f "$PROJECT_ROOT/setup.sh" ]]; then
        log "Running setup.sh to fetch DXNN models..."
        bash "$PROJECT_ROOT/setup.sh" 2>&1 | tee -a "$LOG_DIR/setup_${TIMESTAMP}.log"
        if [[ ${PIPESTATUS[0]} -eq 0 ]]; then
            log "✓ setup.sh completed successfully"
        else
            log "ERROR: setup.sh failed to prepare models"
            exit 1
        fi
    else
        log "ERROR: setup.sh not found. Please provide DXNN models manually under engine/models/"
        exit 1
    fi
else
    log "✓ DXNN model folders present"
fi
...
log "=== Benchmark Run Confirmation ==="

# Ask user whether to run benchmark; default is NO after 10 seconds
USER_INPUT=""
read -t 10 -p "Run benchmark now? [y/N] (auto-no in 10s): " USER_INPUT || true
if [[ ! "$USER_INPUT" =~ ^[Yy]$ ]]; then
    log "Benchmark skipped by user or timeout."
    log "You can run later with: bash startup.sh"
    # Provide quick summary and exit early
    log "Environment ready. Models and dependencies verified."
    echo "Benchmark skipped. Exiting."
    exit 0
fi
...
run_sync_benchmark() {
    ...
    local benchmark_cmd="python -u scripts/dxnn_benchmark.py --directory images --ground-truth images/labels.json --output output_sync --runs 1 --save-individual --mode sync"
...
run_async_benchmark() {
    ...
    local benchmark_cmd="python -u scripts/dxnn_benchmark.py --directory images --ground-truth images/labels.json --output output_async --runs 1 --save-individual --mode async"
