...
static int onRunAsyncCallback(dxrt::TensorPtrs &outputs, void *userArg)
{
    // The outputs is only valid within this callback function

    // TO DO

    // Use userArg to match with the input sequence
    int *pSeq = reinterpret_cast<int*>(userArg);
    LOG_DXRT << "[TEST][CB] seq=" << *pSeq << std::endl;

    // delete user agrument
    delete pSeq;
...
static int bitMatchAsync(std::string modelPath, std::string modelFolderPath, int loopCount)
{
    ...
    try {
        ...
        for (int i = 0; i < loopCount; i++)
        {
            ...
            // Use new or malloc to create a variable so it can be accessible within the callback function"
            int* pSeq = new int(i);

            // run(inference) asynchronously
            // The input data must remain valid until the callback is invoked
            inferenceEngine.RunAsync(inputPtr.get(), reinterpret_cast<void*>(pSeq));
