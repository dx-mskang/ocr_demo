#!/bin/bash
# Check: [BUILD] USE_SHARED_DXRT_LIB=OFF (@aarch64)
TARGET_SOURCE_DIR=$1
...
# Use sed to find and replace 'USE_SHARED_DXRT_LIB' line containing 'OFF' with 'ON'
sed -i 's/\(USE_SHARED_DXRT_LIB.*\)ON/\1OFF/' "$CONFIG_FILE"
